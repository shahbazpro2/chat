...
const LeftSide = ({ chats, groups, filteredMessages, setActiveClick, chatLoading }) => {
    ...
    console.log('gorup', checkActiveUser, pinnedUsers, activeChat, searchUsers, pinnedUsers, ((!checkActiveUser && !search) && !activeChat?.group && !pinnedUsers?.find(p => p.id === activeChat?.id) && !activeChat?.members))

    return (
        <div className='bg-white rounded-lg py-10 h-full relative'>
            {
                chatLoading ? <CircularProgress className='absolute top-1/4 left-1/2' /> : <>
                    <div className="pb-2">
